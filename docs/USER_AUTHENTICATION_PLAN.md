...
- User IDs are now UUIDs (migration 006 completed 2025-12-13)
...
### Migration 006: Convert User IDs to UUIDs ✅ COMPLETE

**Completed:** 2025-12-13

**Why this was needed:** The schema used `id = lowercase(username)` which broke when users sign up via different services (Discogs username ≠ Last.fm username). Needed stable UUIDs for user IDs.

**What was done:**
- 14 users migrated from username-based IDs to UUIDs
...
- `username` field now required (NOT NULL UNIQUE) in schema

**Migration file:** `packages/db/src/migrations/006_uuid_user_ids.sql`
...
-- 2. To rollback (if needed), reverse the process:
-- Insert old user rows, update FKs back, delete new rows
-- Or use D1 point-in-time recovery via Cloudflare dashboard
```

**Code changes made:**
- `packages/db/src/index.ts` - Added `createUser()` and `isUsernameAvailable()` methods
- `packages/db/src/schema.ts` - Made `username: string` required (was `string | null`)

**Cache note:** Old cache keys using username-based IDs will expire naturally in 6-24 hours.

---

### Migration 007: Add Authentication Fields (Future)
